...
/* TRANSLATOR Gui::DelcamNavigationStyle */

TYPESYSTEM_SOURCE(Gui::DelcamNavigationStyle, Gui::UserNavigationStyle)

DelcamNavigationStyle::DelcamNavigationStyle() = default;

DelcamNavigationStyle::~DelcamNavigationStyle() = default;

const char* DelcamNavigationStyle::mouseButtons(ViewerMode mode)
{
    switch (mode) {
    case NavigationStyle::SELECTION:
        return QT_TR_NOOP("Press left mouse button, SHIFT adds to, CTRL removes from selection");
    case NavigationStyle::PANNING:
        return QT_TR_NOOP("Press SHIFT and middle mouse button");
    case NavigationStyle::DRAGGING:
        return QT_TR_NOOP("Press middle mouse button");
    case NavigationStyle::ZOOMING:
        return QT_TR_NOOP("Press CTRL and middle mouse button, scroll middle mouse button alternate");
    default:
        return "No description";
    }
}

SbBool DelcamNavigationStyle::processSoEvent(const SoEvent * const ev)
{
    ...
    if (type.isDerivedFrom(SoMouseButtonEvent::getClassTypeId())) {
        ...
        case SoMouseButtonEvent::BUTTON1:
            ...
            else if (press && (curmode == NavigationStyle::SEEK_WAIT_MODE)) {
                newmode = NavigationStyle::SEEK_MODE;
                this->seekToPoint(pos); // implicitly calls interactiveCountInc()
                processed = true;
                this->lockrecenter = true;
            }
            else if (press && (curmode == NavigationStyle::IDLE)) {
